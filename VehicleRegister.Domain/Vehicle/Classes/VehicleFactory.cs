                               ...
                               IVehicleService bookedService, DateTime firstUseInTraffic)
        {
            switch (weight)
            {
                case double w when (w > 1800 && w < 2500):
                    return new MediumVehicle(vehicleId, registrationNumber, model, brand, weight, isRegistered,
                                             bookedService, firstUseInTraffic);
                case double w when (w >= 2500):
                    return new HeavyVehicle(vehicleId, registrationNumber, model, brand, weight, isRegistered,
                                            bookedService, firstUseInTraffic);
                default:
                    return new LightVehicle(vehicleId, registrationNumber, model, brand, weight, isRegistered,
                                            bookedService, firstUseInTraffic);
            }
        }
